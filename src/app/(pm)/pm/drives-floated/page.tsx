  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-[13px] text-muted mt-1">Every drive posted from this account · {drives.length} total</p>
      </div>
            ...
            <Card key={d.id}>
              <div className="flex items-start justify-between flex-wrap gap-3 mb-3">
                <div>
                  ...
                  <p className="text-xs text-muted">{d.profiles_offered.join(" · ")}</p>
                </div>
                <div className="text-right text-xs text-muted">
                  <div>Posted {new Date(d.posted_on).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}</div>
                  <div>Deadline: {new Date(d.last_date_of_application).toLocaleString("en-IN")}</div>
                </div>
              </div>
